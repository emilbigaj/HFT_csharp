//BEGIN_FILE HFT/Tools/AtomicEnum.cs
using System;
...
// fails â€” a thread that slept through state changes cannot act on the world it remembers,
// even if the state byte has cycled back to what it saw (ABA).
//
// Contract: one owner thread calls Store() with plain authority â€” it may overwrite a
...
// snapshot taken when the evidence for the transition was observed â€” never a fresh Load(),
// which would adopt the new epoch and defeat the check.
public struct AtomicEnum<TState> where TState : unmanaged, Enum
{
    ...
    public AtomicEnum(TState state)
    {
        if (Unsafe.SizeOf<TState>() != 1)
            throw new NotSupportedException($"AtomicEnum<{typeof(TState).Name}>: enum must be byte-backed.");
        _word = Pack(0, state);
    }
    ...
    // Any thread: transition only if the word still IS the snapshot â€” same state, same
...
//END_FILE HFT/Tools/AtomicEnum.cs
